...
| `.gitignore` | Ignores `.env`, `.venv`, and other standard artifacts. |
...
1. **Install dependencies** (creates a virtual environment and installs from the lockfile):
   ```bash
   uv sync
   ```
2. **Add your API key** — create a `.env` file in the project root:
   ...
   ```
3. **Open the notebook** — launch `claude-api.ipynb` in Jupyter / VS Code and run the cells.

## Note on the model

The notebook currently uses `claude-sonnet-4-0`, which the SDK flags as deprecated (EOL June 15, 2026). To keep these examples working, consider migrating to a current model such as `claude-sonnet-4-6` or `claude-opus-4-8`.
